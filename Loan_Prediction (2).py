...
# Buat form input sesuai fitur penting (contoh beberapa fitur saja)
with st.form("prediction_form"):
    ...
    person_home_ownership = st.selectbox("Status Tempat Tinggal", data['person_home_ownership'].unique())
    person_emp_length = st.number_input("Lama Bekerja (tahun)", min_value=0, max_value=50, value=5)
    loan_intent = st.selectbox("Tujuan Pinjaman", data['loan_intent'].unique())
    loan_grade = st.selectbox("Grade Pinjaman", data['loan_grade'].unique())
